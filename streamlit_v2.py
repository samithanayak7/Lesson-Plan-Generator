...
from schedule_classes import schedule_from_data
...
# ----------------------------------------------------
# HELPER: Build a display-friendly table for the class schedule
# ----------------------------------------------------

def build_schedule_table(course_plan, unit_key):
    """
    course_plan looks like:
        {"sheet_name": ..., "unit_1": [{"class_1": [...], "class_2": [...]}], ...}
    Returns a DataFrame with one row per class: Class | Topics
    """
    unit_dict = course_plan[unit_key][0]
    rows = []
    for class_name, topics in unit_dict.items():
        rows.append({
...
if txt_file is not None and pdf_file is not None:
    ...
    if "skeleton" in st.session_state and st.session_state["skeleton"]:
        ...
        if st.button("📖 Extract Syllabus"):
            ...
            try:
                ...
                # A fresh extraction invalidates any previously generated
                # schedule, so clear it -- the user needs to re-generate
                # against the new hours.
                st.session_state.pop("course_plan", None)
                st.session_state.pop("schedule_report", None)
    ...
    if "sections" in st.session_state:
        ...
        # ----------------------------------------
        # Class scheduling (schedule_classes.py)
        # ----------------------------------------

        st.divider()
        st.header("🗓️ Class Schedule")
        st.caption(
            "Turns the hours above into an actual lecture-by-lecture plan. "
            "Number of classes per unit is fixed at round(total unit hours); "
            "each topic is shown only in the class where it needs the most time."
        )

        if st.button("📅 Generate Class Schedule"):
            try:
                course_plan, schedule_report = schedule_from_data(
                    st.session_state["full_json"],
                    sheet_name="course_plan"
                )
        ...
        if "course_plan" in st.session_state:
            course_plan = st.session_state["course_plan"]
            schedule_report = st.session_state["schedule_report"]

            # Quick sanity-check summary: hours vs classes per unit
            report_df = pd.DataFrame(schedule_report).rename(
                columns={
                    "unit": "Unit",
                    "total_hours": "Total Hours",
                    "num_classes": "Classes Scheduled",
                }
            )
            st.subheader("Summary")
            st.dataframe(report_df, use_container_width=True, hide_index=True)

            # One tab per unit, each showing its class-by-class breakdown
            unit_keys = [k for k in course_plan.keys() if k != "sheet_name"]
            tabs = st.tabs([
                schedule_report[i]["unit"] for i in range(len(unit_keys))
            ])

            for tab, unit_key in zip(tabs, unit_keys):
                with tab:
                    schedule_df = build_schedule_table(course_plan, unit_key)
                    st.dataframe(
                        schedule_df,
                        use_container_width=True,
                        hide_index=True
            ...
            with st.expander("🔍 View raw course plan JSON"):
                st.json(course_plan)

            st.download_button(
                label="⬇️ Download Course Plan (JSON)",
                data=json.dumps(course_plan, indent=2),
