...
from topic_grouper import group_similar_topics
...
if txt_file is not None and pdf_file is not None:
    ...
    if "sections" in st.session_state:
        ...
        # ----------------------------------------
        # Group similar topics into whole-hour sessions
        # ----------------------------------------

        st.divider()
        st.header("🔗 Group Similar Topics")
        st.caption(
            "Merges small, thematically related topics (e.g. Bit fields, "
            "Unions, Enums) into single whole-hour lecture sessions, and "
            "rounds standalone topics to the nearest whole hour, while "
            "keeping each unit's total hours unchanged."
        )

        if st.button("🧩 Group Similar Topics (Whole Hours)"):
            try:
                with st.spinner("Asking Gemini which topics to group, then rounding hours..."):
                    grouped_data = group_similar_topics(
                        st.session_state["full_json"],
                        unit_limits
                    )

                grouped_sections = parse_json_sections(grouped_data)

                st.session_state["grouped_json"] = grouped_data
                st.session_state["grouped_sections"] = grouped_sections
            except Exception as e:
                st.error(f"❌ Grouping failed: {e}")

        if "grouped_sections" in st.session_state:

            grouped_topic_df, grouped_unit_totals_df = build_hours_table(
                st.session_state["grouped_sections"]
            )

            if grouped_topic_df.empty:
                st.info("No grouped topics to display.")
            else:
                col_c, col_d = st.columns([2, 1])

                with col_c:
                    st.subheader("Grouped topic-by-topic hours")
                    st.dataframe(
                        grouped_topic_df,
                        use_container_width=True,
                        hide_index=True
                ...
                with col_d:
                    st.subheader("Total hours per unit (grouped)")
                    st.dataframe(
                        grouped_unit_totals_df,
                        use_container_width=True,
                        hide_index=True
                    )
                    st.markdown(
                        f"**Grand total: {int(grouped_topic_df['Hours'].sum())} hrs**"
                    )

                grouped_hours_csv = grouped_topic_df.to_csv(index=False)
                st.download_button(
                    label="⬇️ Download Grouped Hours Table (CSV)",
                    data=grouped_hours_csv,
                    file_name="grouped_topic_hours.csv",
                    mime="text/csv"
                )

            st.success("🎉 Topics Grouped Successfully")

            with st.expander("🔍 View grouped JSON"):
                st.json(st.session_state["grouped_json"])

            st.download_button(
                label="⬇️ Download Grouped JSON",
                data=json.dumps(st.session_state["grouped_json"], indent=4),
                file_name="grouped_syllabus.json",
                mime="application/json"
            )
